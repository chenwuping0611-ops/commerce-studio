...
def reference_instructions(descriptors):
    ...
    for index, descriptor in enumerate(descriptors or [], 1):
        ...
        lines.append(
            f"第 {index} 张参考图 URL：{url}；角色：{label}{suffix}；来源：{source}。"
        )
    return "\n".join(lines)


def video_reference_instructions(urls):
    """Describe ordered video references supplied for the current task."""

    lines = []
    for index, url in enumerate(split_urls(urls), 1):
        lines.append(
            f"第 {index} 个参考视频 URL：{url}；来源：本次上传。"
        )
    return "\n".join(lines)
...
def compose_prompt(
    ...
    Video prompts intentionally use the product identity plus Product Profile,
    product memory, generation rules, and forbidden rules as constraints. The
    broader product description is kept for image creation where it is useful
    as visual merchandising context.
    """
    ...
    if product:
        ...
        if media_type != "VIDEO" and product.description:
    ...
    video_instructions = video_reference_instructions(video_urls)
